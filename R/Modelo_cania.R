...
data_modelo$muj_vlr = as.numeric(data_modelo$muj_vlr)
data_modelo$hom_vlr = as.numeric(data_modelo$hom_vlr)
...
#peqp_vlr+cagrop_vlr+lkdtrab_vlr+linver_vlr+lnorm_vlr
heckman_modelo <- heckit(acceso_credito~pcc,protvd ~ linver_vlr,input_modelo)
resumen_heckman = summary(heckman_modelo)


# 2. Segundo modelo de sample selection
selection_modelo <- selection(acceso_credito~pcc,protvd ~ linver_vlr+lnorm_vlr,input_modelo)
...
modelo_clasico = plm(protvd~qtotal_cred+lkdtrab_vlr+lnorm_vlr+pcc, index=c(anio,municipio), model="random", data=input_modelo)
