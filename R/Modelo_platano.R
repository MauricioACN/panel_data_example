...
data_modelo = read.csv('data/data_platano.csv',sep = ';', dec = ',')
...
#eliminar variables con NA's o ceros

columnas_na = names(data_modelo)[colSums(data_modelo) == 0]
...
heckman_modelo <- heckit(acceso_credito~pcc,protvd ~ peqp_vlr+cagrop_vlr+lkdtrab_vlr+linver_vlr+lnorm_vlr,input_modelo)
resumen_heckman = summary(heckman_modelo)

# 2. Segundo modelo de sample selection
selection_modelo <- selection(acceso_credito~pcc,protvd ~ peqp_vlr+cagrop_vlr+lkdtrab_vlr+linver_vlr,input_modelo)
...
modelo_clasico = plm(protvd~peqp_vlr+credes_vlr, index=c(anio,municipio), model="random", data=input_modelo)
